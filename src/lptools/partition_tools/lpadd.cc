...
bool SuperHelper::AddPartition(const std::string& partition_name, const std::string& group_name,
    ...
    if (!image_path.empty()) {
        raw_image_fd.reset(open(image_path.c_str(), O_RDONLY | O_CLOEXEC | O_BINARY));
        if (raw_image_fd < 0) {
...
bool SuperHelper::OpenSuperFile() {
    ...
    output_fd_.reset(open(actual_path.c_str(), O_RDWR | O_CLOEXEC | O_BINARY));
    if (output_fd_ < 0) {
